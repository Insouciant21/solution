...
int cmp1(Group x, Group y) {
	if (x.contri == y.contri) return x.id < y.id;
	else return x.contri > y.contri;
}
...
int main() {
	...
	for (int i = 1; i <= n; i++) {
		if (i == 1) xingyue[i].newpos=BangZhu;
		else if (i == 2 || i == 3) xingyue[i].newpos = FuBangZhu;
		else if (i == 4 || i == 5) xingyue[i].newpos = HuFa;
		else if (i >= 6 && i <= 9) xingyue[i].newpos = ZhangLao;
		else if (i >= 10 && i <= 16) xingyue[i].newpos=TangZhu;
		else if (i >= 17 && i <= 41) xingyue[i].newpos=JingYing;
		else xingyue[i].newpos=BangZhong;
	}
