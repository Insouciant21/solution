...
void buildTree(int pos, int start, int end, Node *last_node, bool left) {
    ...
    Node *t = new Node;
    t->id = Post[pos];
    if (left) last_node->left = t;
    else last_node->right = t;
    int rightNum = end - k;
...
void dfs(Node *f, int sum) {
    ...
    if (f->left == nullptr && f->right == nullptr && sum + f->id <= res) {
        if (sum + f->id < res) ans = f->id;
        if (sum + f->id == res) ans = min(f->id, ans);
        res = min(res, sum + f->id);
    }
    if (f->left != nullptr) dfs(f->left, sum + f->id);
    if (f->right != nullptr) dfs(f->right, sum + f->id);
}
